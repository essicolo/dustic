...
{#snippet tile(t: Theme)}
	<button
		...
		aria-label={$_('components.themePicker.useTheme', { values: { name: t.name } })}
	...
	>
		...
		<div class="flex items-center gap-2">
			...
			>
				{$_('components.themePicker.playSwatch')}
			</span>
			<span style={mockMutedStyle(t)} class="text-xs">{$_('components.themePicker.artistSongSwatch')}</span>
		</div>
	</button>
{/snippet}
		...
		<div class="bg-base-100 rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl">
			...
			<div class="flex justify-end">
				<button class="btn btn-primary" on:click={confirm}>
					{$_('components.themePicker.continueWith', { values: { name: THEME_LIST.find((t) => t.id === selected)?.name ?? '' } })}
				</button>
			</div>
		</div>
	</div>
