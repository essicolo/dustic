<script lang="ts">
	// Moved from /about (formerly a sidebar nav item) into the Settings
	// tab strip alongside Preferences / Profile / Audio sources, so the
	// sidebar can drop a redundant top-level entry.
...
<div class="space-y-6">
	...
	<section class="card bg-base-200 p-6 border-l-4 border-primary">
		...
		<div class="space-y-3 text-base-content/80 leading-relaxed">
			...
			<p class="mt-4">{$_('about.pwa.outro')}</p>
		</div>
	</section>
	...
	<section class="card bg-base-200 p-6">
		...
		<div class="space-y-4 text-base-content/80 leading-relaxed">
			<div>
				...
				<a href="https://archive.org" target="_blank" rel="noopener noreferrer" class="link link-primary text-sm">
			...
			<div>
				...
				<a href="https://funkwhale.audio" target="_blank" rel="noopener noreferrer" class="link link-primary text-sm">
					{$_('about.sources.fwLink')}
				</a>
			</div>
			<p class="text-sm text-base-content/60">{$_('about.sources.footer')}</p>
		</div>
	</section>
	...
	<section class="card bg-base-200 p-6 border-l-4 border-info">
		...
		<p class="text-base-content/80 leading-relaxed">{$_('about.disclaimer.body')}</p>
	</section>
	...
	<section class="card bg-base-200 p-6">
		...
		<div class="space-y-3 text-base-content/80 leading-relaxed">
			...
			<p class="mt-3"><strong>{$_('about.privacy.outroStrong')}</strong>{$_('about.privacy.outroRest')}</p>
		</div>
	</section>
	...
	<section class="card bg-base-200 p-6 border-l-4 border-warning">
		...
		<div class="space-y-3 text-base-content/80 leading-relaxed">
			...
			<p class="font-semibold">{$_('about.backup.strong')}</p>
			<p>{$_('about.backup.outro')}</p>
		</div>
	</section>
	...
	<section class="card bg-base-200 p-6">
		...
		<div class="space-y-3">
			...
			<a href="https://github.com/essicolo/dustic/issues/new" target="_blank" rel="noopener noreferrer" class="btn btn-outline w-full flex items-center gap-2">
				<Icon icon="solar:chat-round-dots-bold" width="20" />
				<span>{$_('about.support.feedback')}</span>
			</a>
		</div>
		<p class="text-sm text-base-content/70 mt-4">{$_('about.support.footer')}</p>
	</section>
</div>
